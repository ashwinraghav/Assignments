...
class function EvalTool.Create(
  const Hook: ITokenHandler;
  const Stream: IStream;
...
begin
  ...
  Attributes := ParseExpression(
    SilLexer.Tool.TokenLexer(Stream, Tokens, SilSfEvalTokens.DefineTokens(SilLexer.Dictionary.Create())),
    Result.Instructions,
    Attributes);
end;
...
begin
  Result := TSilEvaluator.Create(Hook);
  Attributes := ParseExpression(
    SilLexer.Tool.TokenLexer(Code, Tokens, SilSfEvalTokens.DefineTokens(SilLexer.Dictionary.Create())),
    Result.Instructions,
    Attributes);
end;

class function EvalTool.Execute(
  const Hook: ITokenHandler;
  const Code: String;
