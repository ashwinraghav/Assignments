...
type
  ...
  private
    ...
    function DoGet(const Item: PVariant): Variant;
    procedure DoPut(const Item: PVariant; const Value: Variant);
  ...
  public
    constructor Create(const Name: string; const Parent: ILayerConfigScope);
...
constructor TSilLayerConfigScope.Create(const Name: string; const Parent: ILayerConfigScope);
begin
  inherited Create;
  FName := Name;
  FParent := Parent;
...
begin
  ...
  if Result then Value := DoGet(Item);
end;
...
function TSilLayerConfigScope.GetName: string;
begin
  Result := FName;
end;
...
begin
  Item.Name := Name;
  Item.Value := TSilLayerBinding.Create(IID, Getter, Setter) as ILayerBinding;
  FList.Add(@Item)
end;
...
begin
  Item.Name := Name;
  Item.Value := TSilLayerBinding.Create(IID, nil, Setter) as ILayerBinding;
  FList.Add(@Item)
end;
...
function TSilLayerConfigScope.DoGet(const Item: PVariant): Variant;
var
  Bind: ILayerBinding;
begin
  if Assigned(Item) then
    if Sil.Vart.IsObject(Item^) and Sil.Vart.ToInterface(Item^, ILayerBinding, Bind) then
      Result := Bind.Getter() else
      Result := Item^
  else
    Result := Sil.Vart.Unassigned;
end;

procedure TSilLayerConfigScope.DoPut(const Item: PVariant; const Value: Variant);
var
  Bind: ILayerBinding;
begin
  if Assigned(Item) then
    if Sil.Vart.IsObject(Item^) and Sil.Vart.ToInterface(Item^, ILayerBinding, Bind) then
      Bind.Setter()^ := Value else
      Item^ := Value;
end;
