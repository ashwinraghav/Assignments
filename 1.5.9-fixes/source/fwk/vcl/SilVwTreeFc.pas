...
type
  ...
  private
    ...
    procedure ViewDeletion(TreeView: TfcCustomTreeView; Node: TfcTreeNode);
//    procedure ViewDeletion(Sender: TObject; Node: TfcTreeNode);
  ...
  public
    constructor Create(View: TfcTreeView);
  ...
  public
    constructor Create(Tree: TTreeView; Node: TfcTreeNode);
    destructor Destroy; override;
  end;
...
constructor TTreeView.Create(View: TfcTreeView);
begin
  inherited Create;
  Assert(View <> nil);
  FView := View;
...
procedure TTreeView.ViewDeletion(TreeView: TfcCustomTreeView; Node: TfcTreeNode);
//procedure TTreeView.ViewDeletion(Sender: TObject; Node: TTreeNTNode);
var
  Temp: TTreeNode;
begin
  if Assigned(FOnDeletion) then FOnDeletion(TreeView, Node);
  Temp := Node.Data;
  if Temp <> nil then
  begin
    Temp.FNode := nil;
...
constructor TTreeNode.Create(Tree: TTreeView; Node: TfcTreeNode);
begin
  inherited Create;
  Assert(Node <> nil);
  FTree := Tree;
