...
type
  TSilDelphiTypeDef = class(TSilTypeDef)
  private
    FInfo: PTypeInfo;
  protected // ITypeDef
...
const
  MTypeClasses: array[TTypeKind] of TSilTypeDefClass =
    (
      ...
      TSilDelphiTypeShortString,    // tkString,
...
function TSilDelphiTypeDef.GetTypeInfo: PTypeInfo;
begin
  Result := FInfo;
end;
...
begin
  while inherited Enumerate(Enum, Item) do
    if Sil.Text.Compare(Item.Name, Name) = 0 then
    begin
      Result := True;
      Exit;
    end;
...
function TSilDelphiInterfaceMethod.GetTypeInfo: PTypeInfo;
begin
  Result := nil;
end;
