...
function VarIsRef(const V: Variant): Boolean;
...
function VarIsEmpty(const V: Variant): Boolean;
begin
  Result := System.VarIsEmpty(V);
end;

function VarIsNull(const V: Variant): Boolean;
begin
  Result := System.VarIsNull(V);
end;

function VarIsArray(const V: Variant): Boolean;
begin
  Result := System.VarIsArray(V) or ((VarType(V) = varVariant) and VarIsRef(V) and VarIsArray(Variant(TVarData(V).VPointer^)));
end;
...
function VarAsType(const V: Variant; AVarType: TVarType): Variant;
begin
  Result := System.VarAsType(V, AVarType);
end;

function VarCompareValue(const A, B: Variant): TVariantRelationship;
begin
  if Vart.IsGreater(A, B, false) then
    Result := vrGreaterThan
  else if Vart.IsLower(A, B, false) then
    Result := vrLessThan
  else
    Result := vrEqual;
end;
...
function VarArrayCreate(const Bounds: array of Integer; AVarType: TVarType): Variant;
begin
  Result := System.VarArrayCreate(Bounds, AVarType);
end;
...
function VarArrayLowBound(const A: Variant; Dim: Integer): Integer;
begin
  Result := System.VarArrayLowBound(A, Dim);
end;

function VarArrayHighBound(const A: Variant; Dim: Integer): Integer;
begin
  Result := System.VarArrayHighBound(A, Dim);
end;

function VarArrayDimCount(const A: Variant): Integer;
begin
  Result := System.VarArrayDimCount(A);
end;

function VarArrayLock(const A: Variant): Pointer;
begin
  Result := System.VarArrayLock(A);
end;

procedure VarArrayUnlock(const A: Variant);
begin
  System.VarArrayUnlock(A);
end;
