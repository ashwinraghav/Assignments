...
type
  WindowsTimeZone = class (SilTimeZone)
...
class function WindowsTimeZone.GetTimeZoneInfo(out Name: String; out Bias: Integer): Boolean;
var
...
class function WindowsTimeZone.GlobalToLocal(const Value: TDateTime): TDateTime;
var
  LT, UT: TSystemTime;
begin
...
class function WindowsTimeZone.LocalToGlobal(const Value: TDateTime): TDateTime;
var
  lpZone: TTimeZoneInformation;
  LT: TDateTime;
  UT: TSystemTime;
begin
  OsError.Check(GetTimeZoneInformation(lpZone) <> $FFFFFFFF, 'Windows.GetTimeZoneInformation');

  LT := Value;
  DateTime.Inc(LT, 4, lpZone.Bias);
  DateTimeToSystemTime(LT, UT);
