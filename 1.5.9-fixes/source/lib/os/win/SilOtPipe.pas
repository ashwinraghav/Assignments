...
type
  WindowsNamedPipe = class(SilNamedPipe)
...
class function WindowsNamedPipe.Create(const Name: String;
...
begin
  Result := TWindowsNamedPipeServer.Create(TSilPipeParameters.Create(Name, Access, PipeType, ReadMode, MaxInstances, OutBufferSize, InBufferSize, TimeOut));
end;

class function WindowsNamedPipe.Open(const Name: String;
  const Access: TPipeAccess; const PipeType, ReadMode: TPipeMode): INamedPipeClient;
begin
  Result := TWindowsNamedPipeClient.Create(TSilPipeParameters.Create(Name, Access, PipeType, ReadMode));
end;
