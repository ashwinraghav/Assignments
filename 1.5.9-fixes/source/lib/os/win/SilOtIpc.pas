...
type
  WindowsIpcTool = class(SilIpcTool)
...
class function WindowsIpcTool.CriticalSection: ICriticalSection;
begin
  Result := TSilOsCriticalSection.Create;
end;

class function WindowsIpcTool.Event(const Name: PChar): IEvent;
begin
  Result := TSilOsEvent.Create(Name);
end;

class function WindowsIpcTool.Event(ManualReset, InitialState: Boolean; const Name: PChar): IEvent;
begin
  Result := TSilOsEvent.Create(ManualReset, InitialState, Name);
end;

class function WindowsIpcTool.Mutex(InitialOwner: Boolean; const Name: PChar): IMutex;
begin
  Result := TSilOsMutex.Create(Name, InitialOwner);
end;

class function WindowsIpcTool.Mutex(const Name: PChar): IMutex;
begin
  Result := TSilOsMutex.Create(Name);
end;

class function WindowsIpcTool.Semaphore(InitialCount, MaxCount: Integer; const Name: PChar): ISemaphore;
begin
  if MaxCount = 0 then MaxCount := System.MaxInt;
  Result := TSilOsSemaphore.Create(Name, InitialCount, MaxCount);
end;

class function WindowsIpcTool.Semaphore(const Name: PChar): ISemaphore;
begin
  Result := TSilOsSemaphore.Create(Name);
end;

class function WindowsIpcTool.SharedMemory(const Size: LongWord; const Name: PChar): ISharedMemory;
begin
  Result := TSilOsSharedMemory.Create(Name, Size);
end;

class function WindowsIpcTool.SharedMemory(const Name: PChar): ISharedMemory;
begin
