...
type
  WindowsComputerTool = class(ComputerTool)
...
class function WindowsComputerTool.Local: ILocalComputer;
begin
  Result := TSilOsLocalComputer.Create;
end;

class function WindowsComputerTool.Remote(const Name: string): IComputer;
begin
  Result := TSilOsRemoteComputer.Create(Name);
end;

class function WindowsComputerTool.KindToStr(const Kind: TComputerKinds): string;
begin
  Result := DoKindsToStr(Kind, ';');
end;

class function WindowsComputerTool.KindToStr(const Kind: TDomainKind): string;
begin
  Result := EnumTool.Name(TypeInfo(TDomainKind), Ord(Kind), 'dk');
end;

class function WindowsComputerTool.MacToStr(const Address: LargeInt): string;
begin
  Result := SilOfComputer.DoMacToStr(Address);
end;

class function WindowsComputerTool.Shutdown(const Flags: TShutdownFlags): Boolean;
const
