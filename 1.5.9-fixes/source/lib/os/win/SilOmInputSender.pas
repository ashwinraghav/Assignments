...
{ TInputSender }
  ...
  TInputSender = class(
    ...
    procedure KeyboardSendText(const Text: string; AppendReturn: boolean);
...
begin
	inherited Create;
	FList := Sil.List.PointerList(False, ObjectHandler);
end;
...
procedure TInputSender.KeyboardSendText(const Text: string; AppendReturn: boolean);
var
  i: integer;
begin
// si en el teclado está apretada la tecla Shift, simula que se levanta (es por los números)
  KeyboardUp(VK_LSHIFT);
// envía las teclas que forman el texto
  for i:= 1 to Length(Text) do KeyboardDown(Ord(Text[i]), True);
// agrega el Return final
  if AppendReturn then KeyboardDown(VK_RETURN, True);
  Execute();
end;
