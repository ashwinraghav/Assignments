...
type
  SilWindowsPerformanceTool = class(SilPerformanceTool)
...
{ SilWindowsPerformanceTool }

class function SilWindowsPerformanceTool.Create: IPerformanceCounter;
begin
  Result := TSilOsPerformance.Create();  
end;

class function SilWindowsPerformanceTool.ToDateTime(const Counter: IPerformanceCounter; Reset: Boolean; const Offset: TDateTime): TDateTime;
begin
  Result := DateTime.FromSecs(ToSeconds(Counter, Reset), Offset);
end;

class function SilWindowsPerformanceTool.ToMSeconds(const Counter: IPerformanceCounter; Reset: Boolean): Double;
begin
  Result := 1000 * Counter.GetTime(Reset) / Counter.Frequency;
end;

class function SilWindowsPerformanceTool.ToSeconds(const Counter: IPerformanceCounter; Reset: Boolean): Double;
begin
